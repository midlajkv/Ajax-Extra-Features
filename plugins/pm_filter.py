...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            InlineKeyboardButton('♥️ 𝚂𝙾𝚄𝚁𝙲𝙴 𝙲𝙾𝙳𝙴 ♥️', url='https://github.com/chemban-reju')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            await message.reply_photo(photo="https://telegra.ph/file/6c127809303cd272487b1.jpg", caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
        except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty):
            pic = imdb.get('poster')
            poster = pic.replace('.jpg', "._V1_UX360.jpg")
            await message.reply_photo(photo="https://telegra.ph/file/6c127809303cd272487b1.jpg", caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
        except Exception as e:
            logger.exception(e)
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply_sticker("CAACAgUAAxkBAAEB4HBiDIXHY8y1Pt-MWROq_-1vVUYS4gAC9AMAAjkmQFcPbtzSMCrvrx4E",
                    reply_markup=InlineKeyboardMarkup(btn))
